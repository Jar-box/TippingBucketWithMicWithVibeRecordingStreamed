...
SATURATION_TIP_INTERVAL_S = 15.0  # Δt threshold for hydraulic saturation (seconds)
                ...
                if tip_count > previous_tip_count:
                    ...
                    if last_tip_dt_ms > 0:
                        ...
                        if tip_interval_s >= VALID_TIP_INTERVAL_MIN_S:
                            ...
                            # Check for saturation (Δt ≤ 15s OR intensity ≥ threshold)
                            if tip_interval_s <= SATURATION_TIP_INTERVAL_S or bucket_intensity_mm_hr >= SATURATION_THRESHOLD_MM_HR:
                                saturation_flag = True

                            # Store calibration pair if valid, unsaturated, with enough samples
                            if interval_rms > 0 and len(interval_sample_buffer) >= 5 and not saturation_flag:
                                ...
                                # Mid-session refit: update coefficients live
                                if len(calibration_pairs) >= MIN_PAIRS_FOR_FIT:
                                    _rms = np.array([p[0] for p in calibration_pairs])
                                    _int = np.array([p[1] for p in calibration_pairs])
                                    _c = np.polyfit(_rms, _int, 1)
                                    calib_A, calib_B = float(_c[0]), float(_c[1])
                                    calib_loaded = True
                            elif saturation_flag:
                                pair_quality_reason = "saturated"
